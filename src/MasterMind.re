...
type score = { exactMatches: int, colorMatches: int }
...
let find = (predicate: (int, 'a) => bool, l: list('a)) :option((int, 'a)) => {
  let rec findRec = (index: int, l: list('a)) => {
    switch(l) {
    | [] => None
    | [a, ...rest] => predicate(index, a) ? Some((index, a)) : findRec(index+1, rest)
    }
  }
  findRec(0, l)
}
...
let countAllMatches = (guess: list(codePeg), secret: list(codePeg)) => {
  ...
  let counter = (agg: (int, int), guessPeg: codePeg) => {
    let (index, count) = agg;
    let found = find((i, p) => p === guessPeg && !Array.get(secretMatchTracker, i), secret)
    switch(found) {
    | None => (index + 1, count)
    | Some((index, _)) => {
